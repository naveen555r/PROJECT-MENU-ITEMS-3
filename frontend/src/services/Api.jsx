...
const API_URL = "https://project-menu-items-3backend.onrender.com";
...
export const getMenuItems = async (menuId) => {
  try {
    const response = await axios.get(`${API_URL}/api/menus/${menuId}/items`);
    return response.data; // Return the response data
  } catch (error) {
...
export const addMenuItem = async (menuId, itemData) => {
  try {
    const response = await axios.post(
      `${API_URL}/api/menus/${menuId}/items`,
      itemData,
      {
        headers: {
          "Content-Type": "application/json",
...
export const updateMenuItem = async (menuId, itemId, itemData) => {
  try {
    const response = await axios.put(
      `${API_URL}/api/menus/${menuId}/items/${itemId}`,
      itemData,
      {
        headers: {
          "Content-Type": "application/json",
